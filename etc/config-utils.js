...
import { loadJSON, resolvePath } from "./helpers.js";
...
export async function loadAndValidateConfig(projectRoot, configPath) {
    ...
    const resolveIfSet = (p) => (p ? resolvePath(absProjectRoot, p) : null);

    // get absPath for config-level default fragments (fragments level 0)
    if (config.fragments) {
        for (const [k, v] of Object.entries(config.fragments)) {
            (typeof v === "string" && (v.startsWith("graft:") || v.startsWith("ext:"))) ?
                config.fragments[k] = v : config.fragments[k] = resolveIfSet(v);
        }
    }
        ...
        for (const [name, profile] of Object.entries(config.profiles)) {
            ...
            if (profile.fragments) {
                for (const [k, v] of Object.entries(profile.fragments)) {
                    if (typeof v === "string" && (v.startsWith("graft:") || v.startsWith("ext:"))) {
